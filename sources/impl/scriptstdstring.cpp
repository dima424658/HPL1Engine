...
// AngelScript signature:
// int string::opCmp(const string &in) const
static bool StringEql(const string& a, const string& b)
{
	return !a.compare(b);
}
...
void RegisterStdString_Native(asIScriptEngine *engine)
{
	...
	r = engine->RegisterObjectMethod("string", "bool opEquals(const string &in) const", asFUNCTIONPR(StringEql, (const string &, const string &), bool), asCALL_CDECL_OBJFIRST); assert( r >= 0 );
